...
-- ============================================================================
-- [2026-07-27] ictyb_kepco_dep/user → its_kepco_dep/user 테이블명 접두어 통일 (사용자 확인)
--
-- 이 두 테이블만 "ictyb_" 접두어였고 나머지(its_kdn_dep, its_user_info_secret 등)는
-- 전부 "its_" 접두어라 명명 규칙에 안 맞았다. sql.init.mode=always라 이 스크립트는
-- 서버가 뜰 때마다 재실행되므로, 아래 CREATE TABLE IF NOT EXISTS 앞에서 기존 이름의
-- 테이블이 있으면(=예전 DB) 새 이름으로 RENAME하고, 이미 새 이름이면(=이번 마이그레이션을
-- 이미 거친 DB) 그대로 두고, 애초에 둘 다 없으면(=완전히 새 DB) 아무 것도 하지 않는다
-- (재기동마다 반복 실행되어도 안전). PREPARE/EXECUTE로 조건부 DDL을 흉내낸 이유는 이
-- 스크립트가 DELIMITER 변경 없이 ';' 단위로 파싱되어 IF문을 가진 프로시저를 못 쓰기 때문.
-- ============================================================================
SET @its_kepco_dep_rename_sql = (
    SELECT IF(
        EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'ictyb_kepco_dep')
        AND NOT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'its_kepco_dep'),
        'RENAME TABLE ictyb_kepco_dep TO its_kepco_dep',
        'SELECT 1'
    )
);
PREPARE its_kepco_dep_rename_stmt FROM @its_kepco_dep_rename_sql;
EXECUTE its_kepco_dep_rename_stmt;
DEALLOCATE PREPARE its_kepco_dep_rename_stmt;

SET @its_kepco_user_rename_sql = (
    SELECT IF(
        EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'ictyb_kepco_user')
        AND NOT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'its_kepco_user'),
        'RENAME TABLE ictyb_kepco_user TO its_kepco_user',
        'SELECT 1'
    )
);
PREPARE its_kepco_user_rename_stmt FROM @its_kepco_user_rename_sql;
EXECUTE its_kepco_user_rename_stmt;
DEALLOCATE PREPARE its_kepco_user_rename_stmt;
...
CREATE TABLE IF NOT EXISTS its_kepco_dep (
...
CREATE TABLE IF NOT EXISTS its_kepco_user (
    ...
    SOSOK_CD VARCHAR(16) COMMENT '소속코드(전체, its_kepco_dep.OF_CD 참조)',
...
CREATE TABLE IF NOT EXISTS its_access_log (
    SABUN VARCHAR(10) NOT NULL COMMENT '사번',
...
-- 4-2. IT 작업 보고 삭제 테이블 (지시서_IT부서_삭제)
-- 작업지시서 삭제 시 its_it_work_report의 원본 스냅샷을 옮겨 담는다 (자식 레코드용 DEL 테이블은 없음).
CREATE TABLE IF NOT EXISTS its_it_work_report_del (
    INST_ID VARCHAR(14) NOT NULL COMMENT '처리번호',
    ...
    PRIMARY KEY (INST_ID)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '지시서_IT부서_삭제';
...
CREATE TABLE IF NOT EXISTS ictyb_sales_daily_report (
    ...
    AUTHOR_SABUN VARCHAR(20) COMMENT '최종 수정자 사번',
    AUTHOR_NAME VARCHAR(50) COMMENT '최종 수정자명'
    -- FRST_REG_DT/LST_CHG_DT, REPORT_DATE UNIQUE 제약은 아래 [2026-07-26] ALTER TABLE 블록에서 추가한다.
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '영업 점검일지 헤더 (파트별 결재 도입 이후 날짜당 1행)';
...
CREATE TABLE IF NOT EXISTS ictyb_sales_daily_report_part (
    ...
    PART_ID VARCHAR(20) NOT NULL COMMENT '파트ID (ictyb_part_info.PART_ID)',
    ...
    -- 결재 상태 관련 컬럼(AUTHOR_SABUN, STATUS 등)은 아래 [2026-07-26] ALTER TABLE 블록에서 추가한다.
    PRIMARY KEY (REPORT_ID, PART_ID)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '영업 점검일지 파트별 작성 내용 및 결재 상태';
...
-- 3. 파트 및 사용자 정보
-- ============================================================================
-- [2026-07-27] ybict_part_info/user → ictyb_part_info/user 테이블명 접두어 통일
-- (다른 테이블들처럼 앞이 "ictyb_"가 아니라 글자가 뒤바뀐 "ybict_"였던 오타를 사용자 확인 후 정정)
-- 로직은 위 ictyb_kepco_dep/user → its_kepco_dep/user 마이그레이션과 동일하다.
-- ============================================================================
SET @ictyb_part_info_rename_sql = (
    SELECT IF(
        EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'ybict_part_info')
        AND NOT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'ictyb_part_info'),
        'RENAME TABLE ybict_part_info TO ictyb_part_info',
        'SELECT 1'
    )
);
PREPARE ictyb_part_info_rename_stmt FROM @ictyb_part_info_rename_sql;
EXECUTE ictyb_part_info_rename_stmt;
DEALLOCATE PREPARE ictyb_part_info_rename_stmt;

SET @ictyb_user_info_rename_sql = (
    SELECT IF(
        EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'ybict_user_info')
        AND NOT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'ictyb_user_info'),
        'RENAME TABLE ybict_user_info TO ictyb_user_info',
        'SELECT 1'
    )
);
PREPARE ictyb_user_info_rename_stmt FROM @ictyb_user_info_rename_sql;
EXECUTE ictyb_user_info_rename_stmt;
DEALLOCATE PREPARE ictyb_user_info_rename_stmt;

CREATE TABLE IF NOT EXISTS ictyb_part_info (
...
CREATE TABLE IF NOT EXISTS ictyb_user_info (
    PART_ID VARCHAR(20) NOT NULL COMMENT '파트ID',
    PART_NM VARCHAR(100) COMMENT '파트명',
...
CREATE TABLE IF NOT EXISTS `its_work_result_attach` (
    ...
    CONSTRAINT `FK_its_work_result_TO_attach` FOREIGN KEY (`INST_ID`, `REQ_ID`) REFERENCES `its_work_result` (`INST_ID`, `REQ_ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '결과보고_KDN첨부';

-- 개인정보 포함 여부 (its_it_work_report/its_real_work_report/its_work_result 공용, DOC_TYPE으로 문서 구분)
CREATE TABLE IF NOT EXISTS its_user_info_secret (
    ...
    ATTACH_EXPIRE_DATE VARCHAR(8) COMMENT '파기일',
    OPPB_CL_YN VARCHAR(1) COMMENT '공개구분여부',
    PRIMARY KEY (INST_ID, DOC_TYPE, REQ_ID)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '개인정보포함여부';

-- 요청서_실무부서 (RealWorkReportVo / its_real_work_report)
-- 지시서(its_it_work_report)의 REQ_ID가 이 테이블의 INST_ID를 참조한다. 지금까지는 Hibernate
-- ddl-auto(update)로만 만들어져 있었는데(2026-07-21 "작업 요청서 등록" 기능에서 실제로 쓰기 시작하며 추가).
CREATE TABLE IF NOT EXISTS its_real_work_report (
    ...
    OPPB_YN VARCHAR(1) COMMENT '공개여부',
    PRIMARY KEY (INST_ID)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '요청서_실무부서';

-- 요청서_실무부서_첨부 (RealWorkReportAttachVo / its_real_work_report_attach)
CREATE TABLE IF NOT EXISTS its_real_work_report_attach (
    INST_ID VARCHAR(14) NOT NULL COMMENT '요청번호',
    SEQ VARCHAR(3) NOT NULL COMMENT '순번',
    REAL_FILE_NAME VARCHAR(1000) COMMENT '실제파일명',
    FILE_NAME VARCHAR(1000) COMMENT '변환파일명',
    ...
    FILE_SIZE VARCHAR(10) COMMENT '파일크기',
    PRIMARY KEY (INST_ID, SEQ)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '요청서_실무부서_첨부';

-- 정보시스템정보(단위시스템) 마스터 테이블 (SystemInfoVo / its_system_info)
CREATE TABLE IF NOT EXISTS its_system_info (
...
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '정보시스템정보';

-- 코드 정보 마스터 테이블 (CodeVo / its_code)
CREATE TABLE IF NOT EXISTS its_code (
    ...
    CONSTRAINT PK_ITS_CODE PRIMARY KEY (GUBUN, CODE)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '코드 정보';

-- 코드 정보 마스터 데이터
INSERT IGNORE INTO
    its_code (GUBUN, CODE, CODE_NAME)
VALUES ('01', '05', '업무지원'),
    ('01', '01', '시스템기능개선'),
    ('01', '02', '자료추출'),
    ('02', '11', 'SW유지보수(일반)'),
...
-- 지금까지는 ictyb_part_info의 PAR_DEP_ID로만 참조되고 실제 부서 행은 없었는데,
-- 처장(강명희, 952452)의 로그인 시 부서 조회(KdnDepRepository.findByDepId)를 위해 추가(2026-07-08).
INSERT IGNORE INTO
    its_kdn_dep
VALUES ('9811000', NULL, '영배사업처', 'Y');
...
-- ictyb_user_info엔 있었지만 위 5개 파트 시드 블록에서 빠져 있어 로그인이 불가능했던 인원(2026-07-08 확인).
INSERT IGNORE INTO
    its_kdn_user (
        DEP_ID,
        LOGIN_ID,
...
-- SABUN은 '20' + 기존 8자리(19109330/07104936)로 구성 - KepcoSabunUtil.toLegacySabun()이 앞 2자리를
-- 버리면 다시 19109330/07104936이 나와서, 이 두 사번을 REG_USER_SABUN 등으로 참조하는 기존 시드 데이터와
-- 여전히 매칭된다(2026-07-22 사용자 확인).
-- SOSOK_CD1은 영업배전시스템실 소속 판별 기준값('70ZR')으로 채운다(2026-07-23, 사용자 확인).
VALUES (
        '2019109330',
        '배강우',
        '영업배전시스템실',
...
-- 영배 처장(영업배전시스템실 소속). 처장 판별은 JIKGUB_HAN='1직급' 값 자체로 하므로
-- (2026-07-22, 사용자 확인) JIKGUB_HAN이 채워져 있어도 '1직급'이 아니면 처장이 아니다
-- (예: 나누리/오광식의 '3직급'은 일반 직급명일 뿐 처장이 아님).
INSERT IGNORE INTO
    its_kepco_user (
        SABUN,
        NAME,
...
-- SABUN을 '20'+8자리(97109061) 형태의 10자리로 통일한다(2026-07-23, 사용자 확인: 앞으로 이 테이블은
-- 예외 없이 전부 10자리 사번만 사용). 화면에는 KepcoSabunUtil.toLegacySabun()이 앞 '20'을 떼어낸
-- 8자리(97109061)로 표시되므로 다른 테이블(REG_USER_SABUN 등)의 8자리 참조값과는 계속 일치한다.
-- SOSOK_CD1은 영업배전시스템실 소속 판별 기준값('70ZR')으로 채운다(2026-07-23, 사용자 확인).
VALUES (
        '2097109061',
        '이명종',
...
-- 수요효율처 효율사업실 설비효율담당 (SOSOK_CD4/JIKGUB 미제공 -> 공란)
INSERT IGNORE INTO
    its_kepco_user (
        SABUN,
        NAME,
...
-- ============================================================================
-- [2026-07-23] its_kepco_user.SABUN 10자리 통일 마이그레이션 (자가 치유)
--
-- 배경: 2026-07-22 커밋에서 배강우/정재균의 SABUN을 8자리(19109330/07104936)에서
-- 10자리(2019109330/2007104936)로 바꿨는데, 이 파일의 INSERT는 전부 INSERT IGNORE라
-- 기존 행을 지우지 않는다. sql.init.mode=always라 이 schema.sql은 서버가 뜰 때마다
-- 다시 실행되지만(ddl-auto:update와 함께, 데이터는 안 지워짐) 이미 예전 8자리 행이 쌓여
-- 있던 로컬/개발 DB는 새 10자리 행이 추가로 생기면서 같은 사람이 두 행(8자리+10자리)으로
-- 중복되었다. 그 결과 결재자 후보 목록(WorkMyServiceImpl.requestReceptionCandidates() 등)에
-- 정재균/배강우가 각각 2번씩 뜨는 버그가 발생했다(2026-07-23, 사용자 보고).
--
-- 방침(2026-07-23, 사용자 확인): 이제부터 its_kepco_user 테이블은 SABUN을 예외 없이
-- 전부 10자리('20'+기존 8자리)로만 관리한다. 화면에 노출되는 사번은 여전히 8자리를 유지해야
-- 하므로, 이 테이블에서 값을 읽어 다른 곳(로그인 신원/결재자 표시 등)에 쓸 때는 지금처럼
-- KepcoSabunUtil.toLegacySabun()으로 변환한다 - 이 아래 두 단계는 원본 테이블 자체를
-- 10자리로 맞추는 마이그레이션이고, 변환 로직 자체는 건드리지 않는다.
--
-- 이 블록은 서버 재기동마다 반복 실행되어도 안전(idempotent)하다 - 정리할 8자리 행이
-- 없으면 두 문장 모두 0행에 적용되고 끝난다.
-- ============================================================================

-- 1단계: 이미 대응하는 10자리 행이 있는 예전 8자리 행(=위 마이그레이션으로 중복된 행)을 삭제한다.
-- 예: SABUN='07104936'(구) 옆에 SABUN='2007104936'(신, 같은 사람)이 이미 존재하면 '07104936'을 지운다.
-- (단순 DELETE ... WHERE SABUN IN (SELECT ...)는 MariaDB에서 "같은 테이블을 서브쿼리 대상으로
-- 못 씀" 에러가 나서 자기 자신과의 JOIN으로 우회한다.)
DELETE old_row FROM its_kepco_user old_row
INNER JOIN its_kepco_user new_row
    ON new_row.SABUN = CONCAT('20', old_row.SABUN)
WHERE LENGTH(old_row.SABUN) = 8;

-- 2단계: 1단계 이후에도 남아있는, 대응하는 10자리 행이 없는 "8자리 단독" 행을 10자리로 변환한다.
-- (예: 이명종처럼 아직 신규 10자리 행이 별도로 만들어진 적 없는 사람 - 위 INSERT문 자체를 이미
-- 10자리로 고쳤으므로 정상 부팅 흐름에서는 대개 0행이지만, 이 UPDATE를 아직 못 받은 과거 DB를
-- 위해 남겨둔다.) SABUN이 PK이지만 1단계에서 충돌 가능성이 있는 행을 이미 제거했으므로 안전하다.
UPDATE its_kepco_user
SET SABUN = CONCAT('20', SABUN)
WHERE LENGTH(SABUN) = 8;

-- [2026-07-23] 영업배전시스템실 소속 판별 기준을 SOSOK_HAN(문자열)에서 SOSOK_CD1('70ZR')로 바꾸면서
-- 이 세 사람(배강우/정재균/이명종)의 SOSOK_CD1을 위 INSERT문에서 '9999' 대신 '70ZR'로 고쳤는데,
-- INSERT IGNORE라 이미 '9999' 값으로 떠 있던 로컬/개발 DB에는 반영되지 않는다. 이 UPDATE로 자가 치유한다
-- (재기동마다 반복 실행되어도 안전 - 이미 '70ZR'이면 0행에 적용되고 끝난다).
UPDATE its_kepco_user
SET SOSOK_CD1 = '70ZR'
WHERE SABUN IN ('2019109330', '2007104936', '2097109061')
  AND SOSOK_CD1 <> '70ZR';
...
UPDATE ictyb_user_info SET USER_NM = '이규호' WHERE EMPNO = '300006';

UPDATE ictyb_user_info SET USER_NM = '신희만' WHERE EMPNO = '300007';

UPDATE ictyb_user_info SET USER_NM = '김정태' WHERE EMPNO = '300008';

UPDATE ictyb_user_info SET USER_NM = '강진구' WHERE EMPNO = '300009';

UPDATE ictyb_user_info SET USER_NM = '윤승지' WHERE EMPNO = '300010';

UPDATE ictyb_user_info SET USER_NM = '허희원' WHERE EMPNO = '300011';
...
INSERT IGNORE INTO
    its_it_work_report (
        ...
        WORK_PERIOD,
        WORK_CHANGE_PERIOD,
        WORK_CHANGE_PERIOD_CMT,
        EXPECTED_FINISHED_DT,
        WORK_CODE,
        REQUEST_CODE,
        WORK_START_DT,
        WORK_END_DT,
...
VALUES (
        '3738394',
        '3737563',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청1',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
        0,
    ...
    (
        '3738395',
        '3737564',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청2',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
    ...
    (
        '3738396',
        '3737565',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청3',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
    ...
    (
        '3738397',
        '3737566',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청4',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
    ...
    (
        '3738398',
        '3737567',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청5',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
    ...
    (
        '3738399',
        '3737568',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청6',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
    ...
    (
        '3738400',
        '3737569',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청7',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
    ...
    (
        '3738401',
        '3737570',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청8',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
    ...
    (
        '3738402',
        '3737571',
        '[한전ON] 한전온 월별 회원수 현황 및 요금조회건수 추출 요청9',
        '○ 추출기간 : 2026년 1월 31일, 2월 28일, 3월 20일 기준...',
        '02',
        '075822',
        '12',
...
VALUES (
    ...
    (
        ...
        '이인호',
        '20260323165454',
        '[회수]26-03-23 19:11:44 109/953352/박은석'
    ),
    (
        '3738394',
        '4',
...
SET
    WORKER_SABUN = '300009',
    WORKER_NAME = '강진구',
    WORKER_DEP_CD = '9411400'
WHERE
    INST_ID IN ('DUE0003', 'LNG0003');
-- 통계 건 → 기술/통계 (허희원)
UPDATE its_it_work_report
SET
    WORKER_SABUN = '300011',
    ...
    WORKER_DEP_CD = '9411400'
WHERE
    INST_ID IN ('DUE0004', 'LNG0004');
-- 배전 설비/GIS 건 → 배전1/2/3팀에 임의 배정 (협업자 파트 확정 전까지 임시)
UPDATE its_it_work_report
SET
    WORKER_SABUN = '300008',
    WORKER_NAME = '김정태',
    WORKER_DEP_CD = '9611170'
WHERE
    INST_ID = 'DUE0005';
...
VALUES (
        ...
        '로그인 관련 문의',
        '9611175',
        '로그인 시 사번 입력 후 패스워드가 올바르지 않다고 나옵니다.',
        0,
        '222142',
        '9611175',
        '테스터',
...
VALUES (
    ...
    (
        'CMT20261101000002',
        'OPN20261101000001',
        '충전 완료 시점 기준이며 부분 충전은 kWh 비례 계산으로 처리해 주세요.',
        '07104936',
        '정재균',
        '한전 담당자',
        '07104936',
...
VALUES (
    ...
    (
        'CMT20261101000005',
        'OPN20261101000003',
        '해당 버전으로 확인했습니다. 추가 변경 시 사전 공유 부탁드립니다.',
        '962332',
        '이인호',
        '한전 담당자',
        '962332',
...
INSERT IGNORE INTO
    its_n_sign (INST_ID, SABUN, ACT_ID, NAME)
VALUES (
        'APR0001',
        '07104936',
        '106',
...
-- ============================================================================
-- [2026-07-26] 영업 점검일지 파트별 결재 워크플로우 도입
--
-- 배경: sql.init.mode=always라 이 schema.sql은 서버가 뜰 때마다 Hibernate
-- ddl-auto(update)보다 먼저 실행된다(위 NSignVo 관련 주석 참고). 그래서
-- ictyb_sales_daily_report_part의 CREATE TABLE 정의에 STATUS 등 신규 컬럼을
-- 추가해봤자, 이미 테이블이 존재하는 기존 DB에서는 CREATE TABLE IF NOT EXISTS가
-- 아무 일도 하지 않아 컬럼이 없는 채로 아래 UPDATE가 먼저 실행되어
-- "Unknown column 'STATUS'" 오류로 부팅이 실패한다. ddl-auto가 컬럼을 추가할
-- 때까지 기다릴 수 없으므로, 여기서 명시적으로 ALTER TABLE로 컬럼을 먼저
-- 만들어준다(MariaDB의 IF NOT EXISTS 확장 문법 사용, 이미 있으면 조용히 무시).
ALTER TABLE ictyb_sales_daily_report
    ADD COLUMN IF NOT EXISTS FRST_REG_DT DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '최초 등록 일시',
    ADD COLUMN IF NOT EXISTS LST_CHG_DT DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '최종 변경 일시',
    ADD UNIQUE INDEX IF NOT EXISTS UX_REPORT_DATE (REPORT_DATE);

ALTER TABLE ictyb_sales_daily_report_part
    ADD COLUMN IF NOT EXISTS AUTHOR_SABUN VARCHAR(20) COMMENT '이 파트 작성자 사번',
    ADD COLUMN IF NOT EXISTS AUTHOR_NAME VARCHAR(50) COMMENT '이 파트 작성자명',
    ADD COLUMN IF NOT EXISTS STATUS VARCHAR(20) DEFAULT 'DRAFT' COMMENT '결재 상태',
    ADD COLUMN IF NOT EXISTS SUBMITTED_DT DATETIME COMMENT '제출 일시',
    ADD COLUMN IF NOT EXISTS PART_LEADER_SABUN VARCHAR(20) COMMENT '파트장 결재자 사번',
    ADD COLUMN IF NOT EXISTS PART_LEADER_NAME VARCHAR(50) COMMENT '파트장 결재자명',
    ADD COLUMN IF NOT EXISTS PART_LEADER_APPROVED_DT DATETIME COMMENT '파트장 결재 일시',
    ADD COLUMN IF NOT EXISTS HEAD_SABUN VARCHAR(20) COMMENT '부장 최종결재자 사번',
    ADD COLUMN IF NOT EXISTS HEAD_NAME VARCHAR(50) COMMENT '부장 최종결재자명',
    ADD COLUMN IF NOT EXISTS HEAD_APPROVED_DT DATETIME COMMENT '부장 최종결재 일시',
    ADD COLUMN IF NOT EXISTS REJECTED_BY_SABUN VARCHAR(20) COMMENT '반려자 사번',
    ADD COLUMN IF NOT EXISTS REJECTED_BY_NAME VARCHAR(50) COMMENT '반려자명',
    ADD COLUMN IF NOT EXISTS REJECTED_BY_ROLE VARCHAR(20) COMMENT '반려자 역할: PART_LEADER/HEAD',
    ADD COLUMN IF NOT EXISTS REJECT_REASON VARCHAR(500) COMMENT '반려 사유',
    ADD COLUMN IF NOT EXISTS REJECTED_DT DATETIME COMMENT '반려 일시';

-- 레거시 데이터 백필: 위 STATUS 컬럼 도입 이전에 등록된 행들은 STATUS가 NULL인 채로
-- 남는데, 그대로 두면 화면에 "작성중"처럼 아직 결재가 안 끝난 것으로 보여 혼란을
-- 준다. 이미 등록되어 있던 점검일지는 결재 개념이 없던 시절 내용이므로 승인
-- 완료로 취급한다. STATUS가 NULL인 행만 갱신하므로 매번 재실행돼도 안전하다.
UPDATE ictyb_sales_daily_report_part
