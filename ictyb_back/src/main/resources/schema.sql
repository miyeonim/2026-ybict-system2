...
CREATE TABLE IF NOT EXISTS temp_kepco_dep (
...
CREATE TABLE IF NOT EXISTS temp_kepco_user (
    ...
    SOSOK_CD VARCHAR(16) COMMENT '소속코드(전체, temp_kepco_dep.OF_CD 참조)',
...
INSERT IGNORE INTO temp_kepco_dep (OF_CD, OF_CODE, OF_CD1, OF_CD2, OF_CD3, OF_CD4, OF_HAN1, OF_HAN2, OF_HAN3, OF_HAN4, SER_GU, SER_1, SER_2, SER_3, SER_4, F_BONSA, F_JIKHAL, UPDATEDATE) VALUES
...
INSERT IGNORE INTO temp_kepco_dep (OF_CD, OF_CODE, OF_CD1, OF_CD2, OF_CD3, OF_CD4, OF_HAN1, OF_HAN4, F_BONSA, F_JIKHAL) VALUES
('9999999999999900', '9900', '9999', '9999', '9999', '9900', 'KDN연계(임시)', '영업배전시스템실', '9', 'N');

INSERT IGNORE INTO temp_kepco_user (SABUN, NAME, SOSOK_HAN, SOSOK_CD, SOSOK_CD1, SOSOK_CD2, SOSOK_CD3, SOSOK_CD4) VALUES
...
('YY_1002', '영업일반', '300001', '김문희', NULL, 'Y', 'Y', '2020-01-01', '9999-12-31'),
('YY_1003', '수요', '300002', '박희천', NULL, 'Y', 'Y', '2020-01-01', '9999-12-31'),
('YY_1004', '검침', '300003', '김채윤', NULL, 'Y', 'Y', '2020-01-01', '9999-12-31'),
('YY_1005', '요금/청구', '300004', '정은주', NULL, 'Y', 'Y', '2020-01-01', '9999-12-31'),
('YY_1006', '수금', '300005', '송태정', NULL, 'Y', 'Y', '2020-01-01', '9999-12-31'),
...
-- 위 11명의 이름은 협업자가 지정한 실제 담당자로 갱신될 수 있어, 이미 시드된 환경에도 반영되도록 보정한다
UPDATE ybict_user_info SET USER_NM = '김문희' WHERE EMPNO = '300001';
UPDATE ybict_user_info SET USER_NM = '박희천' WHERE EMPNO = '300002';
UPDATE ybict_user_info SET USER_NM = '김채윤' WHERE EMPNO = '300003';
UPDATE ybict_user_info SET USER_NM = '정은주' WHERE EMPNO = '300004';
UPDATE ybict_user_info SET USER_NM = '송태정' WHERE EMPNO = '300005';
...
UPDATE its_it_work_report SET WORKER_NAME = '김문희' WHERE INST_ID = 'PRT0001';
UPDATE its_it_work_report SET WORKER_NAME = '박희천' WHERE INST_ID = 'PRT0002';
UPDATE its_it_work_report SET WORKER_NAME = '김채윤' WHERE INST_ID = 'PRT0003';
UPDATE its_it_work_report SET WORKER_NAME = '정은주' WHERE INST_ID = 'PRT0004';
UPDATE its_it_work_report SET WORKER_NAME = '송태정' WHERE INST_ID = 'PRT0005';
...
-- ════════════════════════════════════════════════════════════════
-- 결재 기능 테스트용 (2건): 6단계 진행코드(104→106→108→109→111→114)를
-- 처음부터/중간부터 각각 밟아볼 수 있도록 만든 전용 픽스처.
-- APR0001: 방금 작성되어 106(한전 파트장 승인) 대기 중 → 승인을 끝까지 밟아보는 용도
-- APR0002: 109(작업결과보고)까지 끝나 111(KDN 파트장 승인) 대기 중 → 반송(반려) 테스트 용도
-- ════════════════════════════════════════════════════════════════
INSERT IGNORE INTO its_it_work_report (
